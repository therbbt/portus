...
#[tauri::command]
pub fn session_open(
    // Generated by the frontend before this call, so it can subscribe to
    // this id's events first and only then invoke this — see AppState::open
...
/// Called only after the user has explicitly confirmed a changed SSH host
/// key (see `SessionEvent::HostKeyMismatch`) — overwrites the stored key for
/// `host_id` ("host:port") so the frontend's follow-up `session_open` retry
/// succeeds instead of hitting the same mismatch again.
#[tauri::command]
pub fn ssh_trust_host_key(host_id: String, key_base64: String) -> Result<(), String> {
    portus_ssh::trust_host_key(&host_id, &key_base64);
    Ok(())
}
...
pub fn save_config(config: Config) -> Result<(), String> {
    let _guard = portus_core::config::LOCK.lock().unwrap();
...
pub fn save_group(id: Option<Uuid>, name: String, parent_id: Option<Uuid>) -> Result<Config, String> {
    ...
    if let Some(existing) = config.groups.iter_mut().find(|g| g.id == id) {
        ...
        config.groups.push(Group { id, name, parent_id, collapsed: false, sort_order, slug: None });
    }
...
/// Sets (or clears, with `None`) a folder's short tag — see `Group::slug`'s
/// own doc comment for what it's for. Its own command rather than folded
/// into `save_group`, same reasoning as `set_group_collapsed` above: it's
/// a single independent field, not part of the rename/reparent flow.
#[tauri::command]
pub fn set_group_slug(group_id: Uuid, slug: Option<String>) -> Result<Config, String> {
    let _guard = portus_core::config::LOCK.lock().unwrap();
    let mut config = portus_core::config::load().map_err(|e| e.to_string())?;
    if let Some(group) = config.groups.iter_mut().find(|g| g.id == group_id) {
        group.slug = slug;
...
#[tauri::command]
pub async fn sftp_download_file(id: String, path: String, local_path: String, state: State<'_, SftpState>) -> Result<(), String> {
    state
        .get(&id)?
        .download_to_file(&path, std::path::Path::new(&local_path))
        .await
